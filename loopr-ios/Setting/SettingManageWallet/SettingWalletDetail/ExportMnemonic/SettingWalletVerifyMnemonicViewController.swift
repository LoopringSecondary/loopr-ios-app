...
class SettingWalletVerifyMnemonicViewController: UIViewController, MnemonicBackupModeCollectionViewControllerDelegate {
    ...
    override func viewDidLoad() {
        ...
        // mnemonics.shuffle()
        sortedMnemonics = appWallet.mnemonics
        ...
        infoLabel.font = FontConfigManager.shared.getLabelENFont(size: 17)
        ...
        mnemonicsTextView.font = FontConfigManager.shared.getLabelENFont()
    ...
    func collectionViewDidSelectItemAt(indexPath: IndexPath) {
        ...
        if !userInputMnemonics.contains(mnemonic) {
            userInputMnemonics.append(sortedMnemonics[indexPath.row])
            mnemonicCollectionViewController.userSelections.append(indexPath.row)
        }
        
        if userInputMnemonics.count == 1 {
            UIView.animate(withDuration: 0.3, animations: {
                self.undoLastClickButton.alpha = 1.0
            })
        }
        mnemonicCollectionViewController.collectionView?.reloadData()
        mnemonicsTextView.text = userInputMnemonics.joined(separator: "  ")
    }
    ...
    @objc func pressedConfrimButton(_ sender: Any) {
        ...
        if verify() {
            ...
            let defaultAction = UIAlertAction(title: "OK", style: .default, handler: { _ in
                if self.userInputMnemonics.count == 0 {
                    UIView.animate(withDuration: 0.3, animations: {
                        self.undoLastClickButton.alpha = 0.0
                    })
                }
                ...
                self.mnemonicsTextView.text = self.userInputMnemonics.joined(separator: "  ")
            })
            alertController.addAction(defaultAction)
    ...
    func verify() -> Bool {
        guard appWallet.mnemonics.count == userInputMnemonics.count else {
            return false
        }
        
        for i in 0..<appWallet.mnemonics.count {
            guard appWallet.mnemonics[i] == userInputMnemonics[i] else {
                return false
            }
        }
        return true
